                        ...
                        @if(Cart::content()->count() != 0)
                            <form action="#" method="post" class="cart-main">
                                ...
                                <table class="shop_table cart">
                                    ...
                                    @foreach(Cart::content() as $pd)
        
                                    <tr class="cart_item">
        
                                        <td class="product-remove">
                                            <a href="{{ route('cart.delete', ['id' => $pd->rowId]) }}" class="product-del remove" title="Remove this item">
                                        ...
                                        <td class="product-thumbnail">
                                            ...
                                            <div class="cart-product__item">
                                                <a href="#">
                                                    <img src="{{ asset($pd->model->image) }}" alt="product" class="attachment-shop_thumbnail size-shop_thumbnail wp-post-image"
                                                    width="60px" height="50px">
                                                </a>
                                                <div class="cart-product-content">
                                                    <p class="cart-author">Callum Bailey</p>
                                                    <h5 class="cart-product-title">{{ $pd->name }}</h5>
                                                </div>
                                            </div>
                                        </td>
        
                                        <td class="product-price">
                                            <h5 class="price amount">${{ $pd->price }}</h5>
                                        </td>
        
                                        <td class="product-quantity">
        
                                            <div class="quantity">
                                                <a href="{{ route('cart.decr',['id' => $pd->rowId, 'qty' => $pd->qty]) }}" class="quantity-minus">-</a>
                                                    <input title="Qty" class="email input-text qty text" type="text" value="{{ $pd->qty }}" placeholder="1" readonly>
                                                <a href="{{ route('cart.incr',['id' => $pd->rowId, 'qty' => $pd->qty]) }}" class="quantity-plus">+</a>
                                            </div>
        
                                        </td>
        
                                        <td class="product-subtotal">
                                            <h5 class="total amount">${{ $pd->total() }}</h5>
                                        </td>
